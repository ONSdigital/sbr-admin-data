...
class AdminDataControllerTest extends PlaySpec with MockitoSugar with Results with Utilities with ModelProperties {
  ...
  "AdminDataController" must {
    "return a valid result" in {
      val id = "12345678"
      when(mockAdminDataRepository.lookup(Some(date), id)) thenReturn Future(Some(Seq(AdminData(date, id))))
      val resp = controller.lookup(id, Some(dateString), Some(MAX_RESULT_SIZE)).apply(FakeRequest())
    ...
    "return 400 for an invalid id (not correct length)" in {
      val resp = controller.lookup("0", Some("201706"), Some(MAX_RESULT_SIZE)).apply(FakeRequest())
      status(resp) mustBe BAD_REQUEST
      contentType(resp).getOrElse(noContentTypeException) mustBe "application/json"
