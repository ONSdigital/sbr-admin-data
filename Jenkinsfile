...
def copyToHBaseNode() {
    ...
    sshagent(credentials: ["sbr-$DEPLOY_DEV-ci-ssh-key"]) {
        withCredentials([string(credentialsId: "sbr-hbase-node", variable: 'HBASE_NODE')]) {
	       ...
	       ssh sbr-$DEPLOY_DEV-ci@$HBASE_NODE hdfs dfs -put -f sbr-$DEPLOY_DEV-ci@$HBASE_NODE:$DEPLOY_DEV/$MODULE_NAME/lib/ons-sbr-admin-data-*.jar /user/sbr-$DEPLOY_DEV-ci/lib/
